...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    /**
     * 检测producer是否处于运行状态，如果不处于运行状态，则抛出异常
     *
     * @throws MQClientException 若是producer不处于运行状态
     */
    ...
    private SendResult sendDefaultImpl(
        ...
        // 检测是否处于运行状态
        this.makeSureStateOK();
        // 校验消息格式，这里会校验消息 topic长度，消息的body是否为空等信息
        Validators.checkMessage(msg, this.defaultMQProducer);
        // 这里设置调用编号，用于下面 打印日志，标记是否为同一次发送消息
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 同步多次调用，这里如果是同步则是3三次，若是异步就一次
            int timesTotal = communicationMode == CommunicationMode.SYNC ? 1 + this.defaultMQProducer.getRetryTimesWhenSendFailed() : 1;
            // 第几次发送
            int times = 0;
            // 这个string数组用于存储每次发送消息选择 broker名称
            String[] brokersSent = new String[timesTotal];
            // 循环调用发送消息，直到成功
            for (; times < timesTotal; times++) {
                // 这里第二次就不是null了，第一次循环的时候是null
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                // 选择消息发送的队列,若是没有发送的队列，则直接结束循环
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                // 发送消息校验,其实就是个钩子，若是添加了相关钩子可以则可以实现相关逻辑
                // 添加方法 roducer.getDefaultMQProducerImpl().registerCheckForbiddenHook()
                ...
                // 发送消息前的逻辑 参见 producer.getDefaultMQProducerImpl().registerSendMessageHook
                ...
                switch (communicationMode) {
                    case ASYNC:
                        ...
                        // 若是启用了消息压缩，若是后面重发，则会压缩两次，所以这里需要处理一下，把原始消息reset回去
                        // 但是这里有点问题，就是消息在finally里面已经reset了，
